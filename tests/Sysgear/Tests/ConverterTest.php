...
class ConverterTest extends TestCase
{
    ...
    public function testFormat()
    {
        ...
        $formatter->expects($this->once())->method('format')
            ->with(123.123, $procent)->will($this->returnValue('123.12 %'));

        $converter = $this->mock(self::C);
        $this->setProp($converter, 'formatter', $formatter);

        $this->assertSame('123.12 %', $converter->format(123.123, $procent));
    }
    ...
    public function testProcess()
    {
        ...
        $caster = $this->mock('Sysgear\Converter\DefaultCaster', array('cast'));
        $caster->expects($this->once())->method('cast')->with(12.3456, $procent)->will($this->returnValue(12.34));

        $formatter = $this->mock('Sysgear\Converter\DefaultFormatter', array('format'));
        $formatter->expects($this->once())->method('format')
            ->with(12.34, $procent)->will($this->returnValue('12.34 %'));

        $converter = $this->mock(self::C);
        $this->setProp($converter, 'formatter', $formatter);
        $this->setProp($converter, 'srcCaster', $caster);

        $this->assertSame('12.34 %', $converter->process(12.3456, $procent));
    }
    ...
    public function testSerialize()
    {
        ...
        $this->assertEquals('C:17:"Sysgear\Converter":106:{a:3:{'.
            's:9:"srcCaster";s:9:"caster A1";s:9:"dstCaster";s:9:"caster B1";'.
            's:9:"formatter";s:12:"formatter X1";}}', serialize($converter));
    }

    public function testUnserialize()
    {
        $converter = unserialize('C:17:"Sysgear\Converter":106:{a:3:{'.
            's:9:"srcCaster";s:9:"caster A2";s:9:"dstCaster";s:9:"caster B2";'.
            's:9:"formatter";s:12:"formatter X2";}}');

        $this->assertSame('caster A2', $this->getProp($converter, 'srcCaster'));
        $this->assertSame('caster B2', $this->getProp($converter, 'dstCaster'));
