...
class BackupRestorerTest extends TestCase
{
    ...
    public function testMergeIncompleteAsIncomplete()
    {
        ...
        $state2 = 0;
        $find = function($obj) use (&$state2, $storedRoleCompany, $storedUserCompany) {
        ...
        $state3 = 0;
        $that = $this;
        $merge = function($obj) use (&$state3, $that, $storedRoleCompany) {
            if (BackupRestorerTest::$debug) {echo 'merge: ' . get_class($obj) . "\n";}
            switch ($state3++) {
            case 0:    // $obj = Role
                $that->assertEquals(2, $obj->id);
                $that->assertTrue($storedRoleCompany === $obj->company);
                break;

            case 1:    // $obj = User
                $that->assertEquals(1, $obj->id);
                $that->assertEquals(135673, $obj->employer->id);
                $that->assertEquals('userCompany', $obj->employer->name);
                $that->assertEquals('superAdmin123', $obj->roles[0]->name);
                $that->assertTrue($storedRoleCompany === $obj->roles[0]->company);
                break;
            }
            return $obj;
        };
        ...
        $merger->expects($this->exactly(0))->method('merge')->will($this->returnCallback($merge));
        ...
        $that->assertTrue($storedRoleCompany === $user->roles[0]->company);
    }
    ...
    public function testMergeCompleteAsIncomplete()
    {
        ...
        $getMandatoryProperties = function($obj) use (&$state1) {
            ...
            switch ($state1++) {
            case 0:    return array('employer');   // $obj = User
            case 3:    return array('name');       // $obj = Company (missing "name" field)
            default:   return array();
            }
        };
        ...
        $merge = function($obj) use (&$state3, $that) {
            ...
            case 1: $that->assertEquals(13, $obj->id); break; // $obj = Role 13
            case 2: $that->assertEquals(11, $obj->id); break; // $obj = User 11
            }
            return $obj;
        };
    ...
    public function testMergeReferencedAsIncomplete()
    {
        ...
        $getMandatoryProperties = function($obj) use (&$state1) {
            ...
            switch ($state1++) {
            case 0:    return array('employer');   // $obj = User
            case 1:    return array('name');    // $obj = Company
            default:   return array();
            }
        };
        ...
        $merge = function($obj) use (&$state3, $that, $storedUserCompany) {
            if (BackupRestorerTest::$debug) {echo 'merge: ' . get_class($obj) . " ($state3)\n";}
            switch ($state3++) {
            case 0: $that->assertEquals(23, $obj->id); break; // $obj = Role 23
            case 1: $that->assertEquals(21, $obj->id); break; // $obj = User 21
            }
            return $obj;
        };
        ...
        $this->assertTrue($storedUserCompany === $user->employer);
        $this->assertTrue($storedUserCompany === $user->roles[0]->company);
    }
